...
void readFromFile(ItemList *it, int *capacity){
	...
	while(!feof(f)){
		fscanf(f, "%d%d%[^\n]", &it->data[i].weight, &it->data[i].value, it->data[i].name);
...
void printData(ItemList it, int capacity){
	...
	for(int i = 0; i < it.size; ++i){
		...
		printf("|%2d |%-20s|%5d    |%5d    |%6d     |\n", i+1,it.data[i].name, it.data[i].weight, it.data[i].value, it.data[i].solution);
	}
	printf("|---|--------------------|---------|---------|-----------|\n");
...
void createTable(ItemList it, float capacity){
	for(int V = 0; V <= capacity; ++V){
		X[0][V] = min(V / it.data[0].weight, 1);
	...
	for(int k = 1; k < it.size; ++k){
		for(int V = 0;  V <= capacity; ++V){
			int max = F[k-1][V];
			int x = 0;
			int maxItems = min(V / it.data[k].weight, 1);
