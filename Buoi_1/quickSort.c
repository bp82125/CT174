...
int findPivot(RecordType arr[], int bot, int top){
	...
	if(arr[k].key > firstKey) return k; //tim duoc pivot la khoa cua phan tu khac nhau dau tien ke tu phan tu thu nhat
	return bot; //truong hop bot == top thi tra ve chinh no
}
